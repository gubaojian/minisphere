...
vector_t*        list_filenames      (sandbox_t* fs, const char* dirname, bool want_dirs);
path_t*          make_sfs_path       (const char* filename);

sfs_file_t* sfs_fopen      (sandbox_t* fs, const char* path, const char* mode);
void        sfs_fclose     (sfs_file_t* file);
bool        sfs_fexist     (sandbox_t* fs, const char* filename);
...
bool        sfs_fspew      (sandbox_t* fs, const char* filename, void* buf, size_t size);
void*       sfs_fslurp     (sandbox_t* fs, const char* filename, size_t *out_size);
long long   sfs_ftell      (sfs_file_t* file);
size_t      sfs_fwrite     (const void* buf, size_t size, size_t count, sfs_file_t* file);
bool        sfs_mkdir      (sandbox_t* fs, const char* dirname);
bool        sfs_rmdir      (sandbox_t* fs, const char* dirname);
...
bool        sfs_rename     (sandbox_t* fs, const char* filename1, const char* filename2);
bool        sfs_unlink     (sandbox_t* fs, const char* filename);
